  ...
  <div>
        ...
        <table class="min-w-full overflow-hidden rounded-lg bg-white leading-normal shadow-md">
          ...
          <tbody>
            @forelse ($areas as $area)
              <tr>
                <td class="border-b border-gray-200 px-5 py-2 text-center text-sm text-gray-900">{{ $area->id }}
                </td>
                <td class="border-b border-gray-200 px-5 py-2 text-center text-sm text-gray-900">{{ $area->name }}
                </td>
    ...
    <x-modal maxWidth="lg" name="{{ $modalCreateOrUpdate }}" :show="false" focusable>
      <form wire:submit.prevent="save" class="p-6">
        <div class="text-lg font-medium text-gray-900">{{ $form->id ? 'Editar Área' : 'Crear Área' }}</div>
        <div class="mt-4">
